#!/usr/bin/env python3
"""
Control script for USBB-RELAY02 (16c0:05df) HID USB relay board.
...
Usage:
  ...
  python3 relay_control.py off 2
...
def main():
    parser = argparse.ArgumentParser(description="USBB-RELAY02 control")
    parser.add_argument("action", choices=["on", "off", "status"],
                        help="Action to perform")
    parser.add_argument("relay", nargs="?", default=None,
                        help="Relay number (1 or 2) or 'all'")
    ...
    if args.action in ("on", "off"):
        if args.relay is None:
            parser.error("on/off requires a relay number (1, 2, or 'all')")
        if args.relay != "all":
            try:
                r = int(args.relay)
